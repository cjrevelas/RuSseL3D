...
use geometry_mod,     only: numnp, xc, isDirichletFace, nodeBelongsToDirichletFace
...
if (mx_exist.eq.1) then
    ...
    do nn = 1, 2
      if (isDirichletFace(mm,nn)) then
        if (export(exportChainsPerArea, iter, convergence)) then
          ...
          call export_chains_area(nodeBelongsToDirichletFace, planar_cell_of_np(:,mm,nn), "mx", rg2OfMatrixMonomer, lengthMatrix, ns_mx_ed, ds_mx_ed, qmx_final, phi_mx, ww)
        endif
  ...
  do mm = 1, numNanoparticleFaces
    if (export(exportChainsPerArea, iter, convergence)) then
      file_name = ""
      write(file_name,'("o.chains_area_w",I1,"_",I1)') mm, nn
      call export_chains_area(nodeBelongsToDirichletFace, sph_cell_of_np(mm,:), "mx", rg2OfMatrixMonomer, lengthMatrix, ns_mx_ed, ds_mx_ed, qmx_final, phi_mx, ww)
    endif
    if (export(exportAdsorbedFree, iter, convergence)) then
      do kk = 1, numnp
...
if (gr_exist.eq.1) then
    ...
    do nn = 1, 2
      if (isDirichletFace(mm,nn)) then
        ...
        if (export(exportChainsPerArea, iter, convergence)) then
          file_name = ""
          write(file_name,'("o.chains_area_w",I1,"_",I1)') mm, nn
          call export_chains_area(nodeBelongsToDirichletFace, planar_cell_of_np(:,mm,nn), "gr", rg2OfGraftedMonomer, lengthGrafted, ns_gr_ed, ds_gr_ed, qgr_final, phi_gr, ww)
        endif
      endif
    enddo
  enddo
  ...
  do mm = 1, numNanoparticleFaces
    ...
    if (export(exportChainsPerArea, iter, convergence)) then
      file_name = ""
      write(file_name,'("o.chains_area_w",I1,"_",I1)') mm, nn
      call export_chains_area(nodeBelongsToDirichletFace, sph_cell_of_np(mm,:), "gr", rg2OfGraftedMonomer, lengthGrafted, ns_gr_ed, ds_gr_ed, qgr_final, phi_gr, ww)
    endif
  enddo
#ifdef DEBUG_OUTPUTS
