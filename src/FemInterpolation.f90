...
function FemInterpolation(nodeId, x_interp, y_interp, z_interp, uu) result(u_interp)
!--------------------------------------------------------------------------------------------------------------------------------------------!
...
!--------------------------------------------------------------------------------------------------------------------------------------------!
interface
    pure function ToolsMatrixInversion(transf)
    real(8), intent(in), dimension(4,4) :: transf
    real(8),             dimension(4,4) :: ToolsMatrixInversion
    end function
end interface
!--------------------------------------------------------------------------------------------------------------------------------------------!
ll = 3
...
do mm = 1, numElementsOfNode(nodeId)
  ...
  call FemIsPointInElement(xl, x_interp, y_interp, z_interp, lint, sv, volel, inside, shp, numDimensions, numNodesLocalTypeDomain)
  ...
  if (inside==1) then
    ...
    transf_inv = ToolsMatrixInversion(transf)
    ...
    call FemShapeFunctions(lc, xl, numDimensions, numNodesLocalTypeDomain, xsj, shp)
